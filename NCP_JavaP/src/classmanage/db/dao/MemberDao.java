...
public class MemberDao {
	...
	public void saveToDatabase(MemberDto mDto) {
		String sql = "INSERT INTO member (USER_ID, SALT, HASHPW, NAME, JOB) VALUES (?, ?, ?, ?, ?)";
		try {
			conn = DBConnection.getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, mDto.getUserId());
			pstmt.setString(2, mDto.getSalt());
			pstmt.setString(3, mDto.getHashedPw());
			pstmt.setString(4, mDto.getName());
			pstmt.setString(5, mDto.getSelect());
	...
	// 사용자에게 입력받은 ID를 통해 해당 회원의 salt 조회
	public String getSaltbyId(String id) {
		String sql = "SELECT SALT FROM member WHERE USER_ID = ?";
		try {
			conn = DBConnection.getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, id);
	...
	// 사용자에게 입력받은 ID를 통해 해당 회원의 해시화된 비밀번호 조회
	public String getHashPwbyID(String id) {
		String sql = "SELECT HASHPW FROM member WHERE USER_ID = ?";
		try {
			conn = DBConnection.getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, id);
	...
	public MemberDto memberInfo(String id) {
		...
		try {
			...
			if (rs.next()) {
				...
				dto.setSelect(rs.getString("JOB"));
			}
